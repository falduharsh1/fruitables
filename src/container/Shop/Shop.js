  ...
  export default function Shop() {
    ...
    const HandleFiltering = () => {
      ...
      if (sort === "l-h") {
        ...
        F_data.sort((a, b) => a.name.localeCompare(b.name));
      } else if (sort === "z-a") {
        F_data.sort((a, b) => b.name.localeCompare(a.name));
      }

      console.log(fruitData);

      if(fruitData){
        return F_data.filter((v) => v.category === fruitData._id);
      }

      console.log(F_data);      

      const prices = F_data?.filter((v) => (
        v.price <= price
      ))
